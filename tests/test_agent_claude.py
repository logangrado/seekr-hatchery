...
class TestConstructMounts:
    def test_with_task_json(self, home, tmp_path):
        ...
        assert f"{agent.CONTAINER_HOME}/.claude/backups" in dsts
    ...
    def test_backups_is_tmpfs(self, home, tmp_path):
        ...
        assert backups.mode == "tmpfs"
        assert backups.src is None
    ...
    def test_shadow_mounts_credentials_json(self, home, tmp_path):
        ...
        assert str(session_dir / "fake_credentials.json") == creds.src
    ...
    def test_missing_claude_dir(self, tmp_path):
        """~/.claude absent (e.g. CI) — all claude mounts are skipped."""
        session_dir = tmp_path / "session"
        session_dir.mkdir()
        mounts = agent.CLAUDE.construct_mounts(session_dir)
        assert mounts == []
