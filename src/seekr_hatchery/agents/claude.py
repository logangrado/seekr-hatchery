...
class ClaudeBackend(AgentBackend):
    ...
    def construct_mounts(session_dir: Path) -> list[Mount]:
        ...
        if claude_dir.exists():
            mounts.append(Mount(src=str(claude_dir), dst=f"{CONTAINER_HOME}/.claude", mode="rw"))
            # Shadow ~/.claude/backups/ so timestamped credential copies never appear in container.
            mounts.append(Mount(src=None, dst=f"{CONTAINER_HOME}/.claude/backups", mode="tmpfs"))
        task_json = session_dir / "claude.json"
        if task_json.exists():
            mounts.append(Mount(src=str(task_json), dst=f"{CONTAINER_HOME}/.claude.json", mode="rw"))
        fake_creds = session_dir / "fake_credentials.json"
        if fake_creds.exists():
            mounts.append(Mount(src=str(fake_creds), dst=f"{CONTAINER_HOME}/.claude/.credentials.json", mode="rw"))
        return mounts
